...
def parseOpts(opt_def):
    ...
    #            (('--branch','-b'),"NGAdmin version branch"):'5.2'            
    #          }
    ...
    for defline,defValue in opt_def.iteritems():
        ...
        assert type(defline) is tuple, "QA_Optparser: invalid type for 'defline' - check config: line must be like this: ((--help, -h),this is the helptext for this option)"
        paramline=defline[0]
        assert type(paramline) is tuple, "QA_Optparser: invalid type for 'paramline' - check config: line must be like this: (--help, -h)"
        # tuple: (--timeout,-t)
...
if __name__ == '__main__':
    ...
    optDef = {  
                ...
                (('--timeout',  '-t')       ,"Timeout for the console to stays open"):        5,
                (('--branch',   '-b')       ,"NGAdmin version branch"):                   '5.2',
    ...
    # do the magic
    options,arguments= QA_Optparser.parseOpts(optDef)
